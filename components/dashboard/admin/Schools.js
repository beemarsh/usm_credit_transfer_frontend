...
export default function Schools({ schools, total_rows }) {
  ...
  const my_data_grid = (
    <DataGrid
      ...
      rows={isSearchResultsShowing ? search_results : schools}
...
function GridActionButton({ params, handleEditOpen }) {
  ...
  const handleRequest = async () => {
    try {
      const re = await fetch(`${SERVER_URL}/delete_school`, {
        credentials: "include",
        method: "DELETE",
        body: JSON.stringify({
      ...
      enqueueSnackbar({
        message: `Succesfully deleted the school`,
      });
      refreshData();
    } catch (error) {
      if (DEBUG) console.log(error);
...
function UserDialog({ open, onClose, schools, edit = false, defaultVal = {} }) {
  ...
  useEffect(() => {
    ...
    if (edit) {
      set_name({ value: defaultVal?.name, error: "" });
      set_address({ value: defaultVal?.address, error: "" });
      set_code({ value: defaultVal?.code, error: "" });
      set_pre_code({ value: defaultVal?.code, error: "" });
    } else {
      set_name({ value: "", error: "" });
      set_address({ value: "", error: "" });
      set_code({ value: "", error: "" });
    }
  }, [edit, defaultVal]);

  const send_req_user = async () => {
    const school_name_valid = isLongNameValid(name.value);
    const code_valid = isSchoolCodeValid(code.value);
    if (!school_name_valid) {
      set_name((pre) => ({
        ...pre,
    ...
    if (!school_name_valid || !code_valid) {
      return;
    }

    try {
      const re = await fetch(
        `${SERVER_URL}/${edit ? `update_school` : `add_school`}`,
        {
          credentials: "include",
          method: "POST",
          body: JSON.stringify({
      ...
      enqueueSnackbar({
        message: `Succesfully ${edit ? `updated` : `created`} the school`,
      });
      onClose();
      refreshData();
    } catch (error) {
  ...
  return (
    ...
    >
      <DialogTitle>{edit ? `Edit Schools` : `Add New School`}</DialogTitle>
